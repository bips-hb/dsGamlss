...
test_that("output_gamlssDS4", {
  ...
  fitted.partial.residuals <- Z.mat %*% gamma.vect.update
  inner.product <- sum(wt * (partial.residuals - fitted.partial.residuals)^2)

  # output
  expect_equal(outputDS4$nobs, nrow(Z.mat))
  expect_equal(outputDS4$inner.product, inner.product, tolerance = 1e-07)
})
