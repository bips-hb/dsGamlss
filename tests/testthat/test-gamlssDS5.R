...
test_that("output_gamlssDS5", {
  ...
  s.old <- as.matrix(rep(0, times=nrow(gamlss1)))
  s.update <- as.matrix(Z.mat %*% gamma.vect.update)
  ...
  wt <- ifelse(wt<1e-10,1e-10,wt)
  sumofsquares <- sum((s.update[,1] - s.old[,1])^2*wt)
  sumofweights <- sum(wt)
  sumofsmoothers <- sum(wt*apply(s.update,1,sum)^2)
  
  # output
  expect_equal(outputDS5$sumofsquares, sumofsquares, tolerance=1e-07)
  expect_equal(outputDS5$sumofweights, sumofweights, tolerance=1e-07)
  expect_equal(outputDS5$sumofsmoothers, sumofsmoothers, tolerance=1e-07)
  
  # assignments
  expect_equal(temp_mu.s, s.update, tolerance=1e-07)
